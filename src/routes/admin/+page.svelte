	...
	<section class="mx-auto max-w-6xl px-4 py-12 sm:px-6 sm:py-20">
		<div class="max-w-3xl">
			...
			<p class="mt-6 max-w-2xl text-base leading-8 text-muted-foreground">
				Keep the header as a simple admin entry point, then use this dashboard for content actions like
				adding projects and, next, blog posts.
			</p>
		</div>
		...
		<div class="mt-8 grid gap-6 md:grid-cols-2">
			...
			<div class="rounded-[2rem] border border-border bg-card p-8">
				...
				<h2 class="mt-4 text-3xl font-semibold tracking-tight">Add a new blog post</h2>
				<p class="mt-4 text-base leading-8 text-foreground/80">
					This will follow the same admin pattern next, with its own drafting form and publishing
					flow.
				</p>
				<p class="mt-6 text-sm font-medium text-muted-foreground">Coming next</p>
			</div>
		</div>
		...
		<div class="mt-8 rounded-[2rem] border border-border bg-card p-8">
			...
			<div class="mt-8 space-y-4">
				{#each data.projects as project}
					<div class="flex flex-col gap-4 rounded-[1.5rem] border border-border bg-background p-5 sm:flex-row sm:items-center sm:justify-between">
						<div>
